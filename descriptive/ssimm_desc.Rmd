...
# thanks: "We thank the anonymous reviewers, Phillip Ayoub, Filiz Garip, Margaret Peters, Roger Waldinger, the Migration Working Group at UCLA, and participants at the 2021 Annual Meetings of the American Sociological Association and the Population Association of America for their helpful feedback."
---
...
country_tab_ci <- acs_ind_survey %>%
  ...
  summarize(survey_prop(proportion = F, vartype = 'ci'),
            n_same_sex = n()) %>%
  ungroup() %>%
  complete(bpld, same_sex) %>%
  filter(same_sex == T) %>%
  replace(is.na(.), 0) %>%
...
left_join(country_tab_ci, 
          acs_ind %>%
            ...
            group_by(bpld) %>%
            summarize(n = n(),
                      origin_score =  round(weighted.mean(origin_score, na.rm = T, w = perwt), 1))) %>%
  write_csv(here('country_tab_ci.csv'))
...
left_join(state_tab_ci, 
          acs_ind %>%
            ...
            group_by(state) %>%
            summarize(n = n(),
                      state_policy = round(weighted.mean(state_policy, na.rm = T, w = perwt), 1))) %>%
  write_csv(here('state_tab_ci.csv'))

```

```{r country-tab}
country_tab_ci <- read_csv(here('country_tab_ci.csv'))
...
```{r state-tab}
state_tab_ci <- read_csv(here('state_tab_ci.csv'))
...
huxtable::huxreg(robust_list,
  ...
  huxtable::set_caption('OLS regressions of percentage of immigrants in same-sex couples by country of
origin, US state of residence, and survey year.') %>%
...
bind_rows(sim_country) %>%
  write_csv(here('sim_country.csv'))
...
plot1 <- read_csv(here('sim_country.csv'))  %>%
...
plot2 <- read_csv(here('sim_state.csv')) %>%
...
<!-- \newpage -->
<!-- ```{r state-sim-calc, eval = T, include = F} -->
<!-- ## Varying origin score while keeping state score constant (at min and max) -->
<!-- var1 <- 'origin_score' -->
<!-- var <- 'state_policy' -->
<!-- sim_country <- list() -->
