...
bind_rows(
  ...
  acs_count_base %>%
    ...
    filter((yrmarr <= yrimmig | is.na(yrimmig)) & !is.na(yrmarr)) %>%
    group_by(year, cat) %>%
    summarize(n = sum(perwt)),
  acs_count_base %>%
    filter(mixed_citizenship == T & same_sex == T & 
             (yrmarr <= yrimmig | is.na(yrimmig)) & !is.na(yrmarr)) %>%
    group_by(year) %>%
    summarize(n = sum(perwt)) %>%
    mutate(cat = 'Full sample')) %>%
    mutate(group = 'Married before migration'),
  bind_rows(
  acs_count_base %>%
    mutate(cat = case_when(origin_score > 3 ~ 'Progressive',
                             origin_score < 0 ~ 'Regressive')) %>%
  ...
  acs_count_base %>%
    ...
    summarize(n = sum(perwt)) %>%
    mutate(cat = 'Full sample')) %>%
    mutate(group = 'Married after migration')) %>%
...
```{r married-after}
acs_count_mixed_mar <- count_func('(yrmarr >= yrimmig | is.na(yrimmig)) & !is.na(yrmarr)')
acs_count_low_mar <- count_func('((yrmarr >= yrimmig & origin_score < 0 ) | is.na(yrimmig)) & !is.na(yrmarr)')
acs_count_high_mar <- count_func('((yrmarr >= yrimmig & origin_score > 3 ) | is.na(yrimmig)) & !is.na(yrmarr)')

mod_mixed <- acs_count_mixed_mar %>%
  glm(n ~ I(post_2013*same_sex*mixed) + I(post_2013*same_sex) + I(post_2013*mixed) + 
        post_2013 + group_fe + as.factor(year), 
                        data = ., 
                        family = 'quasipoisson') %>%
