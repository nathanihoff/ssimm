...
policy_df <- lapply(policy_list, function(x){
  ...
  mutate(policy = names(policy_list),
         ...
         policy_name = recode(policy, equal_age = 'Equal age of consent', 
  ...
  mutate(policy_name = factor(policy_name, levels = sort({.$policy_name}, decreasing = T)))

policy_df %>%
  ggplot(aes(policy_name, y = estimate)) +
