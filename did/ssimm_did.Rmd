...
To see how the incidence of mixed-citizenship, same-sex unions varies by origin-country LGB policy context, we stratify our sample by this index. We label birthplace-state-group-year cells depending on the average policy index: Cells with a policy score less than 0 get a "regressive" label, while those with a score greater than 3 get a "progressive" label. Cells with U.S. birthplaces do not receive a label and are included in all analyses. We then estimate equation \ref{eq:main} twice more: once for only "progressive" cells (plus U.S. cells), and once for only "repressive" cells (plus U.S. cells). To examine the influence of individual policies, we follow a similar procedure. Instead of stratifying by policy score, we stratify by an indicator for whether individuals in  birthplace-state-group-year cell, had, on average, a given policy in place in their country of origin in their year of immigration.   
...
Finally, our analyses open the door for future investigations to both strengthen the limitations in our research design and expand our understanding of union formation and LGB migration. First, our analysis cannot assess the pathway of how mixed-citizenship, same-sex union came to be. Either the immigrant partner switched visas (e.g., from employment-based to spousal-based) or they came to the U.S. directly from their country-of-origin on a spousal visa. Our theorization suggests it is primarily the latter. Second, this investigation prioritizes interactions between policy and culture between countries to explain increases in mixed-citizenship, same-sex unions. Though macro-level considerations are important to understand union formation, especially for LGB individuals as policies are ever-changing, we do not attend to individual-level characteristics within country. Attending to these characteristics can give insights into whether LGB migrants select on similar attributes as their straight co-nationals when forming unions with U.S. citizens. Moreover, while we do not see significant increases from repressive countries following DOMA, these individuals are still within the data. Who are they? As @kong_2010_chinese documents, it may be the most privileged individuals with a cosmopolitan cultural orientation who end up in relationships with U.S. citizens. If so, this would strengthen the current study by demonstrating that these cultural processes continue at the individual-level as well. Finally, data sets similar to the ACS exist in several other countries that now recognize same-sex partners for immigration purposes. Thus, future work should assess whether our findings are distinct to the U.S. or these trends replicate across all countries when this policy shift occurs. Doing so will help us get a more general understanding of how policy and culture shapes sexual migration for LGB individuals. 
