...
# Scale of migration
## Different- and same-sex couples containing one or two immigrant
```{r}
acs_wide %>%
...
```{r}
acs_wide %>%
  filter(is.na(yrimmig_main) | yrimmig_main - year <= 1) %>%
  filter(is.na(yrimmig_partner) | yrimmig_partner - year <= 1) %>%
  filter(imm_couple != "none") %>%
  mutate(same_sex = ifelse(same_sex == T, 'Same-Sex', 'Different-Sex')) %>%
  group_by(year, imm_couple, same_sex) %>%
  count(wt = hhwt) %>%
