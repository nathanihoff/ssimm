...
Table \ref{tab:tab:same-couple-laws} looks at the influence of LGBT laws related to couples rather than more generally.

```{r same-couple-laws}
same_dyad_couple1 <- lm(same_prop ~ state_couple_laws, 
                       data = acs_dyad_policy)
same_dyad_couple2 <- lm(same_prop ~ state_couple_laws + origin_score, 
                       data = acs_dyad_policy)
same_dyad_couple3 <- lm(same_prop ~ state_unemploy + state_income + state_couple_laws, 
                       data = acs_dyad_policy)
same_dyad_couple4 <- lm(same_prop ~ state_unemploy + state_income + state + state_couple_laws, 
                       data = acs_dyad_policy)
same_dyad_couple5 <- lm(same_prop ~ state_unemploy + state_income + state + state_couple_laws + origin_score, 
                       data = acs_dyad_policy)




stargazer(same_dyad_couple1, same_dyad_couple2, same_dyad_couple3, same_dyad_couple4,
          same_dyad_couple5,
          header = F, 
          keep = c('origin_score', 'state_couple_laws', 'state_stock_year', 'state_unemploy', 
                   'state_income', 'state_stock_year'),
          add.lines = list(c('State FEs?', 'no', 'no', 'no', rep('yes', 2))),
          keep.stat = c('n', 'rsq'),
          label = 'tab:same-couple-laws',
          title = 'Couple policies: 100*Proportion same-sex in a country-state-year')
```

Table \ref{tab:same-fixed-country} adds country-of-origin variables to test whether they are confounding the effect of LGBT policy score.

```{r same-fixed-country}

same_dyad_country1 <- lm(same_prop ~ state_policy + origin_score + distw + contig + 
                          comlang_off + comlang_ethno + colony +
                          wage_dif + unemp_dif + polity5, 
                       data = acs_dyad_policy)
same_dyad_country2 <- lm(same_prop ~ state_policy + origin_score + distw + contig + 
                          comlang_off + comlang_ethno + colony +
                          wage_dif + unemp_dif + polity5 + Country, 
                       data = acs_dyad_policy)
same_dyad_country3 <- lmer(same_prop ~ state_policy + origin_score + distw + contig + 
                          comlang_off + comlang_ethno + colony +
                          wage_dif + unemp_dif + polity5 + (1 | Country), 
                       data = acs_dyad_policy)
same_dyad_country4 <- lmer(same_prop ~ state_policy + origin_score + distw + contig + 
                          comlang_off + comlang_ethno + colony +
                          wage_dif + unemp_dif + polity5 + 
                            (1 | Country) + (1| state), 
                       data = acs_dyad_policy)
same_dyad_country5 <- lm(same_prop ~ state_policy + origin_score + distw + contig + 
                          comlang_off + comlang_ethno + colony +
                          wage_dif + unemp_dif + polity5 + 
                           state_unemploy + state_income, 
                       data = acs_dyad_policy)
same_dyad_country6<- lm(same_prop ~ state_policy + origin_score + distw + contig + 
                          comlang_off + comlang_ethno + colony +
                          wage_dif + unemp_dif + polity5 + 
                           state_unemploy + state_income + Country + state, 
                       data = acs_dyad_policy)

stargazer(same_dyad_country1, same_dyad_country2, same_dyad_country3,
          same_dyad_country4, same_dyad_country5, same_dyad_country6,
          header = F, 
          keep = all.vars(formula(same_dyad_country5)[-2]),
          add.lines = list(c('State FEs?', rep('no', 5), 'yes'),
                           c('Country FEs?', 'yes', rep('no', 4), 'yes'),
                           c('State REs?', rep('no', 3), 'yes', 'no', 'no'),
                           c('Country REs?', 'no', 'no', 'yes', rep('no', 3))),
          keep.stat = c('n', 'rsq', 'll'),
          label = 'tab:same-fixed-country',
          title = 'Country controls: 100*Proportion same-sex in a country-state-year')


```

Finally, we repeat some of the above analyses with multilevel models in table \ref{tab:state-multi}.

```{r state-multi}
same_dyad_m1 <- lmer(same_prop ~ state_policy + (1 | state), 
                       data = acs_dyad_policy)
same_dyad_m2 <- lmer(same_prop ~ state_policy + origin_score + (1 | state), 
                       data = acs_dyad_policy)
same_dyad_m3 <- lmer(same_prop ~ state_unemploy + state_income + (1 | state), 
                       data = acs_dyad_policy)
same_dyad_m4 <- lmer(same_prop ~ state_unemploy + state_income + state_policy + 
                       (1 | state), 
                       data = acs_dyad_policy)
same_dyad_m5 <- lmer(same_prop ~ state_unemploy + state_income + state_policy + 
                       origin_score + (1 | state), 
                       data = acs_dyad_policy)
same_dyad_m6 <- lmer(same_prop ~ state_unemploy + state_income + state_policy + 
                       origin_score + state_stock_year + (1 | state), 
                       data = acs_dyad_policy)



stargazer(same_dyad_m1, same_dyad_m2, same_dyad_m3, same_dyad_m4, same_dyad_m5,
          same_dyad_m6,
          header = F, 
          keep = c('origin_score', 'state_policy', 'state_stock_year', 'state_unemploy', 
                   'state_income', 'state_stock_year'),
          keep.stat = c('n', 'll'),
          label = 'tab:state-multi',
          title = 'Multilevel models: 100*Proportion same-sex in a country-state-year')
```
...
se_list <- list()
mod_list <- list(ind_basic, ind_controls, ind_origin, ind_interact, ind_fe, ind_fe_interact)
for(mod_num in 1:length(mod_list)){
  se_list[[mod_num]] <- diag(vcovCL(mod_list[[mod_num]], cluster = ~bpld))
}
...
# acs.means.same <- acs_couple_policy %>% filter(same_sex == T) %>%
#   summarize(across(c(age, origin_score, yrimmig, nchild, log_income), mean, na.rm = T))
# acs.modes.same <- acs_couple_policy %>% filter(same_sex == T) %>%
#   summarize(across(c(educ, no_income, sex), Mode))
# 
# acs.means.dif <- acs_couple_policy %>% filter(same_sex == F) %>%
#   summarize(across(c(age, origin_score, yrimmig, nchild, log_income), mean, na.rm = T))
# acs.modes.dif <- acs_couple_policy %>% filter(same_sex == F) %>%
#   summarize(across(c(educ, no_income, sex), Mode))
# 
# 
# #Make a dataframe that's the mode of the factor variables vs. mean - one row data frame
# #Two dataframes, one for same-sex and one for not. And then make two plots.
# plot.dat.same <- bind_rows(rep(list(bind_cols(acs.means.same, acs.modes.same)),14)) %>%
#   mutate(origin_score = -3:10) %>%
#   mutate(same_sex = T)
# 
# plot.dat.dif <- bind_rows(rep(list(bind_cols(acs.means.dif, acs.modes.dif)),14)) %>%
#   mutate(origin_score = -3:10) %>%
#   mutate(same_sex = F)
# # 
# # plot.dat.same <- plot.dat %>%
...
# # Predicted probabilities
# pred.same <- data.frame(predict(ind_interact, newdata=plot.dat.same, interval = "confidence"),
#                         origin_score=plot.dat.same$origin_score,
#                         same_sex = 'Same-Sex')
# pred.dif <- data.frame(predict(ind_interact, newdata=plot.dat.dif, interval = "confidence"),
#                         origin_score=plot.dat.dif$origin_score,
#                         same_sex = 'Different-Sex')
...
ind_interact2 <- polr(state_policy_binned ~ same_sex*(origin_score + sex + age + educ + 
                   nchild + log_income + no_income + yrimmig),
                data = acs_couple_policy_small, Hess = T)
ind_fe2 <- ind_origin <- polr(state_policy_binned ~ same_sex*origin_score + Country,
                data = acs_couple_policy_small, Hess = T)
ind_fe_interact2 <- polr(state_policy_binned ~ same_sex*(origin_score + sex + age + educ +
                   nchild + log_income + no_income) + Country,
                data = acs_couple_policy_small, Hess = T)

se_list <- list()
mod_list <- list(ind_basic2, ind_controls2, ind_origin2, ind_interact2, 
...
stargazer(mod_list,
          ...
          keep = all.vars(formula(ind_interact[-2])),
          se = se_list,
          add.lines = list(c('Country FEs?', 'no', 'no', 'no', 'no', 'yes', 'yes'),
                           c('Origin clust-SEs?', rep('yes', 6))),
          keep.stat = c('n', 'll'), 
          label = 'tab:ind-ord',
...
```{r ord-fig-noci, fig.cap = 'Predicted probability of residing in three LGBT state policy environments', eval = F}
# Predicted probabilities
...
# Plot
ord_plot <- bind_rows(pprobs.same, pprobs.dif) %>%
...
sims <- 250
sim_mod <- list(sims)
pseudeo_mod <- ind_interact2
k <- length(coef(ind_interact2))

for (i in 1:sims) {
  theta <- as.numeric(rmvnorm(1, mean=c(coef(ind_interact2), ind_interact2$zeta),
                              sigma=vcov(ind_interact2)))
...
# Immigrants in past year
This set of analyses considers only those who migrated up to one year before the survey.
...
acs_coupled_imms %>%
  ...
  kable(booktabs = T, caption = 'Numbers of immigrants in different- and same-sex couples who immigrated in the survey year or the year before')

```



```{r state-props, eval = T}
# Loading data that just includes immigrants who came up to a year before the survey
acs_dyad2 <- read.csv(here('data', 'acs_dyad2.csv'))
acs_dyad_policy2 <- acs_dyad2 %>%
  mutate(mean_year_immig = ifelse(mean_year_immig >= 1991, 
                                  round(mean_year_immig),
                                  1991)) %>%
  left_join(lgbt_policy, by = c('mean_year_immig' = 'year', 'bpldid' = 'Code')) %>%
...
acs_dyad_policy2$same_prop <- acs_dyad_policy2$same_sex_stock/acs_dyad_policy2$state_stock_year * 100
acs_dyad_policy2$dif_prop <- acs_dyad_policy2$opp_sex_stock/acs_dyad_policy2$state_stock_year * 100

acs_dyad_policy2 <- acs_dyad_policy2 %>%
  left_join(unemploy,  by = c('mean_year_immig' = 'year', 'state')) %>%
  left_join(state_income,  by = c('mean_year_immig' = 'year', 'state')) 

# apply(acs_dyad_policy2, 2, function(x) sum(is.na(x)))
# sum(complete.cases(acs_dyad_policy2))
# nrow(acs_dyad_policy2)
# nobs(state_prop_fe2)


state_prop <- lm(same_prop ~ origin_score + state_policy, data = acs_dyad_policy2)

state_prop_fe <- lm(same_prop ~ state_unemploy + state_income + state + 
                        origin_score + state_policy, 
                       data = acs_dyad_policy2)
...
state_prop_2 <- lm(same_prop ~ state_policy + origin_score + distw + contig + 
                          comlang_off + comlang_ethno + #colony +
                          wage_dif + unemp_dif + polity5 + 
                           state_unemploy + state_income, 
                       data = acs_dyad_policy2)
...
se_list <- list()
mod_list <- list(state_prop, state_prop_fe, state_prop_fe2, state_prop_2)
...
stargazer(mod_list,
          header = F, 
          keep = c(all.vars(formula(state_prop_fe2)[-2])[1:11]),
          add.lines = list(c('State-clustered SEs?', rep('yes', 4)),
                           c('State FEs?', 'no', 'yes', 'yes', 'no'),
          ...
          title = 'Recent immigrants: 100*Proportion same-sex in a country-state-year')
```

```{r ord}
ind_past_year <- filter(acs_couple_policy, year - yrimmig <= 1)
ord_basic <- polr(state_policy_binned ~ same_sex + as.factor(year),
                data = ind_past_year, Hess = T)
...
ord_interact <- polr(state_policy_binned ~ same_sex*origin_score + sex + age + educ + 
                ...
                data = ind_past_year, Hess = T)
ord_interact2 <- polr(state_policy_binned ~ same_sex*(origin_score + sex + age + educ + 
                   nchild + log_income + no_income + yrimmig) + as.factor(year),
                data = ind_past_year, Hess = T)

se_list <- list()
mod_list <- list(ord_basic, ord_orig, ord_interact2, ord_interact)
for(mod_num in 1:length(mod_list)){
...
stargazer(mod_list,
          ...
          no.space = T,
          label = 'tab:ord',
          title = 'Focus on recent immigrants. Individual ordered logit analysis of three-category state policy score')
```
...
acs.means <- ind_past_year %>% 
  summarize(across(c(age, origin_score, yrimmig, nchild, log_income), mean, na.rm = T))
acs.modes <- ind_past_year %>% 
  summarize(across(c(educ, no_income, sex), Mode))
