...
lgbt_policy <- read_csv(here('data', 'lgbt_policy.csv')) %>%
  select(Country, year, origin_score = total_score)
...
acs_dyad_policy <- acs_dyad %>%
  left_join(lgbt_policy, by = c('mean_year_immig' = 'year', 'bpld' = 'Country')) %>%
