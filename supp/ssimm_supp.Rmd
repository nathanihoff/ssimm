...
ag_2013_quad1 <- lm(prop_same_sex ~ distw + contig + comlang_off + 
                        comlang_ethno + colony +
                   wage_dif + unemp_dif + vdem + origin_score + I(origin_score^2) + stock_prop + bpld,
         data = mutate(acs_prop_yrimmig_policy, post_2013 = (yrimmig > 2013)))
...
stargazer(mod_list,
          ...
          model.names = F,
          keep = c('origin_score', 'post_2013'),
          order = paste0("^", c('origin_score', 'I(origin_score^2)', 'post_2013TRUE',
                                'post_2013TRUE:origin_score') , "$"),
          covariate.labels = c('Country LGB policy score', '(Country LGB policy score)$\\wedge{}$2',
                                'Post-2013', 'Country score × Post-2013'),
          dep.var.labels   = c("Married", "Percentage same-sex"),
