...
stargazer(mod_list,
          ...
          order = paste0("^", c('origin_score', 'origin_squared', 'post_2013TRUE',
                                'post_2013TRUE:origin_score') , "$"),
          covariate.labels = c('Country LGB policy score', '(Country LGB policy score) $\\hat{}$ 2',
                                'Post-2013', 'Country score × Post-2013'),
          dep.var.labels   = c("Percentage same-sex", "Married"),
          add.lines = list(c('Country controls?', rep('yes', 5)),
                           c('Country FEs?', rep('yes', 5))),
          se = se_list,
          keep.stat = c('n'),
          star.cutoffs = c(0.1, 0.05, 0.01, 0.001),
          star.char = c('†', '*', '**', '***'),
