...
```{r desc-ind-figures, fig.cap = 'Individual descriptive statistics for immigrants in couples 2008-2019, with survey weights and 95% confidence intervals. Currency in thousands of 1999 dollars.'}
income <- all_geo %>%
  group_by(year, group) %>%
...
bind_rows(income, hs, college, hwsei, has_children, age) %>%
  rename(Year = year) %>%
...
```{r desc-time-1, fig.height = 6.5, fig.cap = 'Geographic characteristics (percentages) over time for immigrants in different- and same-sex couples, based on American Community Survey data for 2008-2022'}
