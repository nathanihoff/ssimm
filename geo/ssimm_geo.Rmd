...
list('Different-sex, immigrant' = glm(n ~ bachelors_puma + black_puma + hispanic_puma + 
                     log_inctot_puma + owned_puma + age_puma + immigrant_puma +
                     poverty_200_puma  + hwsei_puma + 
                     unemployed_puma + valueh_puma +
  ...
  set_wrap(T) %>%
  set_all_padding(0) %>%
  huxtable::set_caption('Quasi-poisson models by PUMA, regressed on PUMA-level factors, separately by group')
```
