...
The 12 years of survey data contain `r nrow(filter(acs_wide, same_sex == T & (yrimmig_main >= 1991 | yrimmig_partner >= 1991)))` same-sex couples that include at least one immigrant, for a total of `r nrow(filter(acs_couple_policy, same_sex == T))` immigrants in same-sex couples with complete data. These immigrants are compared to `r nrow(filter (acs_wide, same_sex == F & (yrimmig_main >= 1991 | yrimmig_partner >= 1991)))` corresponding different-sex couples containing `r nrow(filter(acs_couple_policy, same_sex == F))` individual immigrants. Below, we outline how we use these data to construct dependent variables based on each analysis. All analyses incorporate ACS sampling weights.  
...
#   select(same_prop, n_dif_sex, n_same_sex)
...
huxreg(robust_list,
  ...
  add_rows(rbind(c('Country controls?', 'no', rep('yes', 4)),
                c('Country FEs?', 'no', 'no', rep('yes', 3))), 
           after = nrow(.) - 2) %>%
  set_caption('OLS regressions of percent of immigrants in same-sex couples by year of immigration and country of origin.') %>%
  set_width(1)
...
huxreg(robust_list,
       ...
       stars = c(`†` = 0.1, `*` = 0.05, `**` = 0.01, `***` = 0.001),
       statistics = c('Observations' = 'nobs')) %>%
  add_rows(rbind(c('State controls and FEs?', 'no', 'no', rep('yes', 5)),
                           c('Country controls and FEs?', 'no', 'no', rep('yes', 5))), 
           after = nrow(.) - 2) %>%
  set_caption('Percent same-sex in by country of origin, U.S. state, and survey year.')%>%
  set_width(1)
...
ord_probs <- bind_cols(predict(ord_basic_controls, type = 'probs'), 
                   same_sex = acs_couple_policy_small$same_sex) %>%
  group_by(same_sex) %>%
...
#                    nchild + ihs_income + no_income + yrimmig,
#                  data = acs_couple_policy_small,  weights = perwt/12)
# 
# 
...
huxreg(robust_list,
  ...
  add_rows(rbind(c('Individual controls?', 'yes', 'yes', 'yes', 'yes'),
                 c('Country controls?', 'no', 'no', 'yes', 'yes'),
                 c('State controls?', 'no', 'no', 'no', 'yes')), 
           after = nrow(.) - 2) %>%
  set_caption('Individual ordered logit analysis of three-category state LGB policy score.')%>%
  set_width(1)
