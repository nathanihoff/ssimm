...
# ag_country_orig_re <- lmerTest::lmer(prop_same_sex ~ distw + contig + comlang_off + comlang_ethno + colony +
#                    wage_dif + unemp_dif + vdem + origin_score + stock_prop + (1|bpld),
#          data = acs_prop)
...
#                    wage_dif + unemp_dif + vdem + origin_score + stock_prop +
#                      post_2013,
#          data = mutate(acs_prop, post_2013 = (yrimmig > 2013)))
...
for (i in 1:sims) {
  ...
  for(cat_loop in unique(acs_prop$post_2013)){
      var_vals <- seq(min(acs_prop[,var]), 
                      max(acs_prop[,var]), 
                      length.out = 50)
      for(val in var_vals){
          sim_dat <- acs_prop %>%
...
state_prop_2013 <- lm(prop_same_sex ~ post_2013 + state_policy + origin_score + distw + contig + 
                          ...
                          wage_dif + unemp_dif + vdem + stock_prop +
                           state_unemploy + state_income + Country + state, 
                       data = mutate(acs_prop_state, post_2013 = (year > 2013)))



state_prop_2013_int2 <- lm(prop_same_sex ~ post_2013*state_policy + origin_score + distw + contig + 
                          comlang_off + comlang_ethno + colony +
                          wage_dif + unemp_dif + vdem + stock_prop +
                           state_unemploy + state_income + Country + state, 
                       data = mutate(acs_prop_state, post_2013 = (year > 2013)))

state_prop_2013_int <- lm(prop_same_sex ~ post_2013*origin_score + state_policy + distw + contig + 
                          comlang_off + comlang_ethno + colony +
                          wage_dif + unemp_dif + vdem + stock_prop +
                           state_unemploy + state_income + Country + state, 
                       data = mutate(acs_prop_state, post_2013 = (year > 2013)))
...
#                        data = mutate(acs_prop_state, post_2013 = (year > 2013)))
# state_prop_2013_2 <- lm(prop_same_sex ~ post_2013*(state_policy + origin_score) + distw + contig + 
#                           comlang_off + comlang_ethno + colony +
...
```{r state-sim, eval = F}
state_prop_fe2 <- glm(prop_same_sex ~ state_policy*origin_score + distw + contig + 
                          comlang_off + comlang_ethno + colony +
                          wage_dif + unemp_dif + vdem + stock_prop +
                           state_unemploy + state_income + Country + state, 
                       data = acs_prop_state, family = 'poisson')
...
#                 data = acs_ind_small, Hess = T, weights = perwt/12)
...
ord_probs <- bind_cols(predict(ord_basic_controls, type = 'probs'), 
                   same_sex = acs_ind_small$same_sex) %>%
  group_by(same_sex) %>%
...
# ord_basic_controls <- lm(state_policy ~ same_sex + sex + age + educ + 
#                    nchild + ihs_income + no_income + yrimmig,
#                  data = acs_ind_small,  weights = perwt/12)
# 
# 
# ord_interact <- lm(state_policy ~ 
#                        same_sex*(origin_score + sex + age + educ + 
#                    nchild + ihs_income + no_income + yrimmig),
#                 data = acs_ind_small, weights = perwt/12)
# 
# ord_interact_country <- lm(state_policy ~ 
#                                 same_sex*(origin_score + sex + age + educ +
#                      nchild + ihs_income + no_income + yrimmig) +
...
#                 data = acs_ind_small,  weights = perwt/12)
# 
# ord_interact_controls <- lm(state_policy ~ 
#                                 same_sex*(origin_score + sex + age + educ +
#                      nchild + ihs_income + no_income + yrimmig) +
#                      distw + contig + comlang_off + comlang_ethno +
#                      wage_dif + unemp_dif + vdem + stock_prop +  bpld + #state +
...
# acs_ind_small %>%
#   group_by(same_sex, married, imm_couple) %>%
#   summarize(mean(origin_score), sd = sd(origin_score), 
#             lower = mean(origin_score) - 1.96*sd,
#             upper = mean(origin_score) + 1.96*sd)
...
sigma_twoway <- vcovCL(ord_interact_controls, cluster = ~ bpld)
...
for (i in 1:sims) {
  ...
  for(same_loop in unique(acs_ind_small$same_sex)){
      var_vals <- seq(min(acs_ind_small[,var]), 
                      max(acs_ind_small[,var]), 
                      length.out = 10)
      for(val in var_vals){
          sim_dat <- acs_ind_small %>%
...
# Tables
```{r analytic}
tibble(`Analytic Strategy` = c(1,2,3),
       Name = c('Country-of-Origin Effects',
                'State Effects',
                'Individual Analysis'),
           `Dependent Variable` = 
             c('Proportion of immigrants in same-sex couples, for each country in each of year of immigration 1991-2019, averaged over the 12 years of survey data',
               'Proportion of immigrants in same-sex couples, for each country, state, and survey year 2008-2019',
               'Binned state policy context for individual immigrants'),
           `Example` = c('Among ACS respondents who immigrated from France in 2001, a weighted 1.3 percent were in same-sex couples at the time of survey.',
                         'Among ACS respondents who immigrated from France and resided in New York at the time of the 2012 survey, a weighted 0.91 percent were in same-sex couples.',
                         'One immigrant from France in ACS survey year 2012 lived in a progressive policy context.'),
           `LGB Policy for Sending Country` = c('linked to year of immigration',
                                     'linked to survey year',
                                     'linked to year of immigration'),
           `LGB Policy for U.S. State` = c('NA',
                                     'linked to survey year',
                                     'linked to survey year')
) %>%
  ...
  # set_caption('Analytic strategies') %>%
  # set_width(1)
  #
  flextable() %>% 
  set_caption('Analytic strategies')  %>%
...
# acs_prop %>%
#   filter(country == 'France', yrimmig == 2001) %>%
#   select(prop_same_sex, n_dif_sex, n_same_sex)
# 
# acs_prop_state %>%
#   filter(country == 'France', year == 2015, state == 'New York') %>%
#   select(prop_same_sex, n_dif_sex, n_same_sex)
...
# robust_list <- append(robust_list, list(ag_country_orig_re), 3)
# 
# huxreg(ag_country_orig_re)
...
huxreg(robust_list,
       coefs = c('State LGB policy score' = 'state_policy',
                 'Country LGB policy score' = 'origin_score', 
                 'State score × country-score' = 'state_policy:origin_score',
                 'Post-2013' = 'post_2013TRUE', 
                 'State score × post-2013' = 'post_2013TRUE:state_policy',
                 'Country score × Post-2013' = 'post_2013TRUE:origin_score'),
       ...
       stars = c(`†` = 0.1, `*` = 0.05, `**` = 0.01, `***` = 0.001),
       statistics = c('Observations' = 'nobs')) %>%
  insert_row(c('State controls and FEs?', 'no', 'no', rep('yes', 5)), after = nrow(.) - 2) %>%
  insert_row(c('Country controls and FEs?', 'no', 'no', rep('yes', 5)), after = nrow(.) - 2) %>%
...
huxreg(robust_list,
       coefs = c('Same-sex' = 'same_sexTRUE', 
                 'Country LGB policy score' = 'origin_score', 
                 'Same-sex × country score' = 'same_sexTRUE:origin_score'),
       ...
       stars = c(`†` = 0.1, `*` = 0.05, `**` = 0.01, `***` = 0.001),
       statistics = c('N (effective)' = 'nobs')) %>%
  insert_row(c('Individual controls?', 'yes', 'yes', 'yes', 'yes'), after = nrow(.) - 2) %>%
  insert_row(c('Country controls?', 'no', 'no', 'yes', 'yes'), after = nrow(.) - 2) %>%
  insert_row(c('State controls?', 'no', 'no', 'no', 'yes'), after = nrow(.) - 2) %>%
  huxtable::set_caption('Individual ordered logit analysis of three-category state LGB policy score.')

# obs <- paste0(substr(nrow(acs_ind_small), 1, 3), ',', 
#               substr(nrow(acs_ind_small), 4, 6))
...
total_df %>%
  mutate(`Total population (thousands)` = n/1000,
         ...
         Year = year,
         imm_couple = ifelse(imm_couple == 'one', 'Mixed-status couples', 'Two-immigrant couples')) %>%
...
wage_dif <- acs_ind_survey %>%
  ...
  mutate(var_name = 'GDP Differential in Year of Migration')
...
country_tab <- acs_ind %>%
  mutate(bpld = ifelse(bpld == "United Kingdom, ns", "England", bpld),
         bpld = ifelse(bpldid == 46500, 'Russia', bpld),
         bpld = ifelse(bpldid == 46510, 'Belarus', bpld)) %>%
  group_by(bpld, same_sex) %>%
  count(wt = perwt) %>%
  pivot_wider(names_from = 'same_sex', values_from = 'n') %>%
  mutate(prop_samesex = round(100*`TRUE` / `FALSE`, 2)) %>%
...
plot1 <- sens_df %>%
  mutate(`Misreporting rate (weighted average)` =
           (mis_spouse*n_spouse+mis_partner*n_partner)/(n_spouse + n_partner)) %>%
  ...
  labs(y = 'Country score × Post-2013', x = 'Misreporting rate') +
  ggtitle('Country Model 5')
...
plot2 <- sens_df %>%
  ...
  theme(legend.position = "none") +
  scale_color_manual(values = c('transparent', 'black')) +
  labs(y = 'State LGB policy score', x = 'Misreporting rate') +
...
plot3 <- sens_df %>%
  ...
  theme(legend.position = "none") +
  scale_color_manual(values = c('transparent', 'black')) +
  labs(y = 'Country LGB policy score', x= 'Misreporting rate') +
  ggtitle('State Model 4')

(plot1 + plot2 + plot3) & theme(text = element_text(size = 9))
```
