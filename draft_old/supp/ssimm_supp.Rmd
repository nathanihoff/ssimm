...
Figure \ref{fig:scatter-country} relies on the data used in the country-level models. The vertical axis represents the proportion of immigrants from each country in same-sex couples in each year of immigration, from 1991 to 2019, while the horizontal axis indicates the policy score of the country of origin in the year of immigration. The line shows the bivariate regression line. Percentages above 5 are not shown (hiding `r sum(acs_prop$prop_same_sex > 5)` out of `r nrow(acs_prop)` country-years).  
...
```{r respmode}
tabyl(acs_ind, respmode, year, same_sex) %>%
...
ag <- lm(prop_same_sex ~ origin_score,
         data = acs_prop, weights = stock_prop)
...
ag_country_orig_fe_2013 <- lm(prop_same_sex ~ distw + contig + comlang_off + comlang_ethno + colony +
                   wage_dif + unemp_dif + vdem + origin_score + stock_prop + bpld +
                     post_2013,
         data = mutate(acs_prop, post_2013 = (yrimmig > 2013)), weights = stock_prop)
...
state_prop_2013 <- lm(prop_same_sex ~ post_2013 + state_policy + origin_score + distw + contig + 
                          ...
                          wage_dif + unemp_dif + vdem + stock_prop +
                           state_unemploy + state_income + Country + state, 
                       data = mutate(acs_prop_state, post_2013 = (year > 2013)), weights = stock_prop)
...
ols_interact <- lm(state_policy ~ 
                ...
                data = rename(acs_ind_small, gender = sex))
...
for(year_loop in 1991:2018){
  placebo_list[[paste0(year_loop, 'simple')]] <- 
    lm(prop_same_sex ~ distw + contig + comlang_off + comlang_ethno + colony +
           ...
           data = mutate(acs_prop, post_year = (yrimmig > year_loop))) %>%
...
for(year_loop in 2008:2018){
  placebo_list[[paste0(year_loop, 'simple')]]  <- 
    lm(prop_same_sex ~ post_year + state_policy + origin_score + distw + contig + 
                          comlang_off + comlang_ethno + colony +
                          wage_dif + unemp_dif + vdem + stock_prop +
                           state_unemploy + state_income + Country + state, 
                       data = mutate(acs_prop_state, post_year = (year > year_loop))) %>%
  ...
  placebo_list[[paste0(year_loop, 'int2')]]  <- 
    lm(prop_same_sex ~ post_year*origin_score + state_policy + distw + contig + 
                            comlang_off + comlang_ethno + colony +
                            wage_dif + unemp_dif + vdem + stock_prop +
                             state_unemploy + state_income + Country + state, 
                         data = mutate(acs_prop_state, post_year = (year > year_loop))) %>%
...
state_prop_fe <- lm(prop_same_sex ~ state_policy + origin_score + distw + contig + 
                       ...
                       data = mutate(acs_prop_state, bpld = Country, sname = state))
...
state_prop_2013 <- lm(prop_same_sex ~ post_2013 + state_policy + origin_score + distw + contig + 
                          ...
                          wage_dif + unemp_dif + vdem + stock_prop +
                           state_unemploy + state_income + bpld + sname, 
                       data = mutate(acs_prop_state, post_2013 = (year > 2013),bpld = Country, sname = state))
